...
File     : modules/05_ipv6_eigrp_ospf_nornir/utils/push_config.py
Purpose  : Send one or more ad-hoc configuration lines to one or more lab
           ...
           troubleshoot_ipv6_eigrp_ospf.py — use it to inject a fault, run
           the troubleshooter to show the symptom, then restore with
           configure_ipv6_eigrp_ospf.py.

           Routers are resolved from ipv6_eigrp_ospf.yaml using the same
...
Usage:
    # Remove EIGRP redistribution on R1 (fault injection)
    python push_config.py --router R1 --cmd "ipv6 router eigrp 100" "no redistribute ospf 1 metric 10000 100 255 1 1500"

    # Remove OSPF redistribution on R6
    python push_config.py --router R6 --cmd "ipv6 router ospf 1" "no redistribute eigrp 111"

    # Dry-run — show commands that would be sent, no SSH connection
    python push_config.py --router R1 --cmd "ipv6 router eigrp 100" "no redistribute ospf 1 metric 10000 100 255 1 1500" --dry-run
...
def main() -> None:
    ...
    EXAMPLES = (
        "Examples:\n"
        "\n"
        "  # Inject redistribution fault on R1 — remove EIGRP->OSPF redistribution\n"
        "  python push_config.py --router R1 --cmd \"ipv6 router eigrp 100\" "
        "\"no redistribute ospf 1 metric 10000 100 255 1 1500\"\n"
        "\n"
        "  # Inject redistribution fault on R6 — remove OSPF->EIGRP redistribution\n"
        "  python push_config.py --router R6 --cmd \"ipv6 router ospf 1\" "
        "\"no redistribute eigrp 111\"\n"
        "\n"
        ...
        "  python push_config.py --router R1 --cmd \"ipv6 router eigrp 100\" "
        "\"no redistribute ospf 1 metric 10000 100 255 1 1500\" --dry-run\n"
        "\n"
        "  # List available routers from YAML\n"
        "  python push_config.py --list-routers\n"
    )
    ...
    parser = argparse.ArgumentParser(
        description=(
            "NAMS26 Module 05 Utility — Ad-hoc Configuration Push\n"
            "Send one or more config lines to one or more lab routers.\n"
            "Designed as a demonstration companion to troubleshoot_ipv6_eigrp_ospf.py."
        ),
        epilog=EXAMPLES,
    ...
    print(f"\n{BOLD}NAMS26 — Module 05: Ad-hoc Config Push [{mode_label}]{RESET}")
