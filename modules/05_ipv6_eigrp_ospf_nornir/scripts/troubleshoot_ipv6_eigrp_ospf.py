...
def check_process(conn, device_name: str, device_data: dict, lf=None) -> str:
    ...
    if ospf_data:
        ...
        # IOL renders OSPFv3 as 'ospf <pid>' in show ipv6 protocols — not 'OSPFv3 <pid>'
        if process_id and re.search(rf'\bospf\s+{re.escape(process_id)}\b', output, re.IGNORECASE):
