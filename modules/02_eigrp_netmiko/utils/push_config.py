...
File     : modules/02_eigrp_netmiko/utils/push_config.py
Purpose  : Send one or more ad-hoc configuration lines to one or more lab
           ...
           troubleshoot_eigrp_classic.py — use it to inject a fault, run the
           troubleshooter to show the symptom, then restore with a second
           push_config.py call.

           Routers are resolved from eigrp_classic.yaml using the same
...
Usage:
    ...
    python push_config.py --router R1 --cmd "no router eigrp 100"

    # Multiple commands — one router
    python push_config.py --router R2 --cmd "router eigrp 100" "no network 10.12.12.0 0.0.0.255"
    ...
    python push_config.py --router R1 --cmd "no router eigrp 100" --dry-run
...
#   modules/02_eigrp_netmiko/
#     data/eigrp_classic.yaml
...
# (r1.lab, R1.lab) — mirrors the alias resolution pattern in troubleshoot_eigrp_classic.py
# =============================================================================
...
def main() -> None:
    ...
    EXAMPLES = (
        ...
        "\n"
        "  # 1. Inject a fault\n"
        "  python utils/push_config.py --router R1 --cmd \"router eigrp 100\" \"no network 10.12.12.0 0.0.0.255\"\n"
        "\n"
        "  # 2. Show the symptom with the troubleshooter\n"
        "  python scripts/troubleshoot_eigrp_classic.py --router R1 --check neighbors routes\n"
        "\n"
        "  # 3. Restore\n"
        "  python utils/push_config.py --router R1 --cmd \"router eigrp 100\" \"network 10.12.12.0 0.0.0.255\"\n"
    )

    parser = argparse.ArgumentParser(
        description=(
            "NAMS26 Utility -- Ad-hoc Configuration Push\n"
            "Send one or more config lines to one or more lab routers.\n"
            "Designed as a demonstration companion to troubleshoot_eigrp_classic.py."
        ),
        epilog=EXAMPLES,
    ...
    print(f"\n{BOLD}NAMS26 — Ad-hoc Config Push [{mode_label}]{RESET}")
