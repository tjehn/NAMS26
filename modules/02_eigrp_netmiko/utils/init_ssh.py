#!/usr/bin/env python3
"""
Module   : NAMS26 — Module 02 Utility
File     : modules/02_eigrp_netmiko/utils/init_ssh.py
Purpose  : Establish an SSH session to each lab router (R1–R11) in sequence,
           ...
           Netmiko scripts can connect without interactive prompts.
...
def check_ssh_router(device_name: str, dns_name: str,
    ...
    ssh_cmd = (
        ...
        # Write accepted keys to the standard known_hosts file so Netmiko scripts
        # can connect without host-key prompts after init_ssh runs.
...
def main() -> None:
    ...
    # Run header
    print(f"\n{BOLD}NAMS26 — Module 02: SSH Lab Initialization{RESET}")
    ...
    if fail_count:
        print(warned(
            ...
            f"Resolve before running Netmiko scripts."
        ))
        sys.exit(1)
    else:
        passed("All routers reachable via SSH. known_hosts is current.")
        print(f"{CYAN}  Ready to run configure_eigrp_classic.py{RESET}\n")
