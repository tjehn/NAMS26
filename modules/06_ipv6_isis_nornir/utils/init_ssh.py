...
File     : modules/06_ipv6_isis_nornir/utils/init_ssh.py
Purpose  : Establish an SSH session to each lab router (BB-1 through OSPF-1)
           in sequence, accepting the host key fingerprint, authenticating,
           confirming the router prompt is reachable, and exiting cleanly.
           Populates ~/.ssh/known_hosts with current host keys so that
           subsequent Nornir scripts can connect without interactive prompts.
...
Usage:
    ...
    python init_ssh.py --router BB-1
...
def init_ssh_router(device_name: str, dns_name: str,
    ...
    Args:
        device_name : Display name (e.g. 'BB-1') — used for output only.
        dns_name    : SSH target hostname (e.g. 'bb-1.lab').
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "NAMS26 Module 06 Utility — SSH Lab Initialization\n"
        ...
        epilog=(
            ...
            "  python init_ssh.py\n"
            "  python init_ssh.py --router BB-1\n"
            "  python init_ssh.py --router ABR-1 ABR-2 ABR-3\n"
        ),
    )
    parser.add_argument(
        "--router",
        ...
        metavar="HOSTNAME",
        help=(
            "Test specific routers only. Accepts BB-1, bb-1, or bb-1.lab. "
    ...
    # Run header
    print(f"\n{BOLD}NAMS26 — Module 06: SSH Lab Initialization{RESET}")
    ...
    for device_name, ok in results.items():
        ...
        if ok:
            print(f"  {GREEN}PASS{RESET}  {device_name:<8}  {dns_name}")
        else:
            print(f"  {RED}FAIL{RESET}  {device_name:<8}  {dns_name}")
