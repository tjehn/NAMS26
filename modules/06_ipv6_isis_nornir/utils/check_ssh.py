...
File     : modules/06_ipv6_isis_nornir/utils/check_ssh.py
...
Usage:
    python check_ssh.py
    python check_ssh.py --host BB-1
    python check_ssh.py --host bb-1.lab
"""
...
def print_result(r: dict) -> None:
    ...
    print(f"  {BOLD}{r['host']:8}{RESET}  {r['dns_name']:16}  "
          f"{color}{BOLD}{status:<10}{RESET}  {r['detail']}")


def main():
    parser = argparse.ArgumentParser(description="Check SSH connectivity to NAMS26 Module 06 lab hosts.")
    parser.add_argument("--host", help="Check a single host by name (BB-1) or DNS name (bb-1.lab).")
    args = parser.parse_args()

    print(f"\n{CYAN}{BOLD}{'─' * 65}")
    print("  NAMS26 — Module 06: SSH Connectivity Check")
    print(f"  Credentials: {USERNAME} / {'*' * len(PASSWORD)}")
    print(f"{'─' * 65}{RESET}\n")
    print(f"  {'HOST':<10}{'DNS NAME':<18}{'STATUS':<12}DETAIL")
    print(f"  {'─' * 62}")
