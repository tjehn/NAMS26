...
File     : modules/06_ipv6_isis_nornir/utils/ping_hosts.py
...
Usage:
    ...
    python ping_hosts.py --host BB-1
    python ping_hosts.py --host bb-1.lab
"""
...
def ping(host: str, dns_name: str, count: int = 3) -> dict:
    ...
    Args:
        host     : Display label (e.g. 'BB-1').
        dns_name : Hostname to resolve and ping (e.g. 'bb-1.lab').
...
def print_result(r: dict) -> None:
    ...
    loss   = r["loss"]
    print(f"  {BOLD}{r['host']:8}{RESET}  {r['dns_name']:16}  {status}   {loss}")


def main():
    parser = argparse.ArgumentParser(description="Ping NAMS26 Module 06 lab hosts.")
    parser.add_argument("--host",  help="Ping a single host by name (BB-1) or DNS name (bb-1.lab).")
    parser.add_argument("--count", type=int, default=3, help="Ping count (default: 3).")
    args = parser.parse_args()

    print(f"\n{CYAN}{BOLD}{'─' * 55}")
    print("  NAMS26 — Module 06: Ping Hosts Utility")
    print(f"{'─' * 55}{RESET}\n")
    print(f"  {'HOST':<10}{'DNS NAME':<18}{'STATUS':<10}PACKET LOSS")
    print(f"  {'─' * 52}")
