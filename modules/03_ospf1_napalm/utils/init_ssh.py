#!/usr/bin/env python3
"""
Module   : NAMS26 — Module 03 Utility
File     : modules/03_ospf1_napalm/utils/init_ssh.py
Purpose  : Establish an SSH session to each lab router (R1–R11) in sequence,
           ...
           NAPALM scripts can connect without interactive prompts.
...
def main() -> None:
    ...
    # Run header
    print(f"\n{BOLD}NAMS26 — Module 03: SSH Lab Initialization{RESET}")
    ...
    if fail_count:
        ...
        print(f"{CYAN}  Ready to run configure_ospf_classic.py{RESET}\n")
