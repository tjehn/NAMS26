...
File     : modules/03_ospf1_napalm/scripts/configure_ospf_classic.py
Purpose  : Generate and apply OSPF Classic Mode configurations to Cisco IOS
...
Usage:
    ...
    python configure_ospf_classic.py --router R1 R2

    # Combine — dry-run on specific routers
    python configure_ospf_classic.py --dry-run --router R1
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Module 03 — OSPF Classic Mode\n"
            "Generates and applies OSPF Classic Mode configurations to Cisco "
            "IOS routers using YAML data and a Jinja2 template via NAPALM."
        ),
    ...
    print(f"\nNAMS26 — Module 03: OSPF Classic Mode [{mode_label}]")
