...
Purpose  : Establish an SSH session to each lab router (R1–R10) in sequence,
           ...
           NAPALM and Netmiko scripts can connect without interactive prompts.
...
Usage:
    ...
    python check_ssh.py --router R1

    # Override credentials (default: netadmin / admin from YAML)
    python check_ssh.py --username netadmin --password admin

Pre-flight requirement:
    Run clear_known_hosts.sh first to remove stale host key entries.
    Running check_ssh.py without clearing first will fail if the stored
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python check_ssh.py\n"
            "  python check_ssh.py --router R1\n"
            "  python check_ssh.py --router R3 R5 R6\n"
        ),
    )
    parser.add_argument(
        "--router",
    ...
    # Run header
    print(f"\n{BOLD}NAMS26 — Module 04: SSH Connectivity Check{RESET}")
    ...
    # Summary
    print("\n" + "=" * 60)
    print(f"{BOLD}  SSH Check Summary{RESET}")
    print("=" * 60)
