#!/usr/bin/env python3
"""
Module   : NAMS26 — Module 04 Utility
File     : modules/04_ospf2_napalm/utils/push_config.py
Purpose  : Send one or more ad-hoc configuration lines to one or more lab
           ...
           troubleshoot_ospf_advanced.py — use it to inject a fault, run the
           troubleshooter to show the symptom, then restore with
           configure_ospf_advanced.py.

           Routers are resolved from ospf_advanced.yaml using the same
...
Usage:
    ...
    python push_config.py --router R1 --cmd "router ospf 1" "no redistribute eigrp 100 metric-type 1 subnets"

    # Multiple commands — one router
    python push_config.py --router R6 --cmd "router ospf 1" "no redistribute eigrp 111 metric-type 1 subnets"

    # Multiple routers — same commands pushed to each
    python push_config.py --router R5 R6 --cmd "router ospf 1" "no area 20 nssa"
    ...
    python push_config.py --router R1 --cmd "router ospf 1" "no redistribute eigrp 100 metric-type 1 subnets" --dry-run
...
#   modules/04_ospf2_napalm/
#     data/ospf_advanced.yaml
...
def main() -> None:
    ...
    EXAMPLES = (
        "Examples:\n"
        "\n"
        "  # Inject redistribution drift on R1 (closing demo)\n"
        "  python push_config.py --router R1 --cmd \"router ospf 1\" "
        "\"no redistribute eigrp 100 metric-type 1 subnets\"\n"
        "\n"
        "  # Inject redistribution drift on R6\n"
        "  python push_config.py --router R6 --cmd \"router ospf 1\" "
        "\"no redistribute eigrp 111 metric-type 1 subnets\"\n"
        "\n"
        "  # Remove NSSA default-information-originate from R3\n"
        "  python push_config.py --router R3 --cmd \"router ospf 1\" "
        "\"area 20 nssa\"\n"
        "\n"
        ...
        "  python push_config.py --router R1 --cmd \"router ospf 1\" "
        "\"no redistribute eigrp 100 metric-type 1 subnets\" --dry-run\n"
        "\n"
        "  # List available routers from YAML\n"
        "  python push_config.py --list-routers\n"
        "\n"
        "Closing demo workflow (redistribution drift on R1):\n"
        "\n"
        "  # 1. Inject drift — remove redistribution on R1\n"
        "  python utils/push_config.py --router R1 --cmd \"router ospf 1\" "
        "\"no redistribute eigrp 100 metric-type 1 subnets\"\n"
        "\n"
        "  # 2. Observe — troubleshooter passes (neighbors and process are healthy)\n"
        "  python scripts/troubleshoot_ospf_advanced.py --router R1 --check neighbors process\n"
        "\n"
        "  # 3. Detect — verifier catches the missing redistributed prefixes\n"
    ...
    parser = argparse.ArgumentParser(
        description=(
            "NAMS26 Module 04 Utility — Ad-hoc Configuration Push\n"
            "Send one or more config lines to one or more lab routers.\n"
            "Designed as a demonstration companion to troubleshoot_ospf_advanced.py."
        ),
        epilog=EXAMPLES,
    ...
    print(f"\n{BOLD}NAMS26 — Module 04: Ad-hoc Config Push [{mode_label}]{RESET}")
