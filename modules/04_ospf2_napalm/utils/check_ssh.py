...
File     : modules/04_ospf2_napalm/utils/check_ssh.py
Purpose  : Verify SSH connectivity to all lab hosts via Netmiko.
           ...
           terminal work without host key mismatch errors. Run this script after
           every lab reboot as part of the standard lab startup sequence.
...
def make_ssh_config() -> str:
    """Write a temp SSH config that accepts new host keys and writes them to
    ~/.ssh/known_hosts. StrictHostKeyChecking is set to 'accept-new' so that:
      - New/updated host keys (e.g. after a lab reboot) are accepted and saved.
      - Known hosts that present a CHANGED key still raise an error, protecting
        against accidental connection to the wrong host.
    This allows manual terminal SSH sessions to work without mismatch errors
...
def check_ssh(host: str, dns_name: str, ssh_config: str) -> dict:
    ...
    A successful connection will write the host key to ~/.ssh/known_hosts."""
    device = {
...
def main():
    parser = argparse.ArgumentParser(description="Check SSH connectivity to NAMS26 Module 04 lab hosts.")
    parser.add_argument("--host", help="Check a single host by name (R1) or DNS name (r1.lab).")
    args = parser.parse_args()

    print(f"\n{CYAN}{BOLD}{'─' * 65}")
    print("  NAMS26 — Module 04: SSH Connectivity Check")
    ...
    try:
        ...
        os.unlink(ssh_config)  # clean up temp file
