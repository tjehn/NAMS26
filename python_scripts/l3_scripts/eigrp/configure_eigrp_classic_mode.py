...
"""Configure EIGRP Classic Mode on lab routers using YAML data and Jinja2 templates.

This script generates and applies EIGRP configurations to Cisco IOS routers via Netmiko.
...
def apply_config_to_router(device_name, device_data, config):
    ...
    router = {
        ...
        "session_log": os.path.join(
            SESSION_LOG_SUBDIR,
            f"session_log_{device_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        ),
    }
    try:
        print(f"Connecting to {device_name} ({oob_ip})...")
        with ConnectHandler(**router) as conn:
            print(f"Connected successfully to {device_name} ({oob_ip})")  # Success message added here
            # Force config mode
...
if __name__ == "__main__":
    ...
    YAML_FILE_PATH = os.path.join(
        PROJECT_ROOT, "labs/lab_eigrp/vars/eigrp_classic_mode.yaml"
    )
    ...
    # Comment/uncomment these as needed for debugging YAML data loading
